...
/* Build Date 12/16/2013 5:05:32 AM */
...
_ActivityList.prototype.Load=function(){var TrackerID='555558';this.TrackingToken=_ltk.GetCookie("_trkt");this.MerchantTrackingID='BJjH6ZK9La7C';this.SessionID=_ltk.SCA.SessionID;if(this.SessionID==null)
...
_Alerts.prototype.Load=function(){this.MerchantTrackingID='BJjH6ZK9La7C';}
...
jQuery("#ltkmodal-content").ltkmodal({closeClass:"ltkmodal-close",overlayClose:self.overlayClose,overlayCss:{backgroundColor:"#000"},overlayId:'ltkmodal-overlay',containerId:'ltkmodal-container',dataId:'ltkmodal-data',onOpen:function(dialog){_ltk.Modal.active=true;dialog.overlay.fadeIn('fast',function(){dialog.container.fadeIn('fast',function(){dialog.data.fadeIn('fast');});});
		dialog.data.find(".ltkmodal-subscribe").click(function(){
			//edit by vtuong 
			// alert("abc");
			var email=jQuery("#ltkmodal-email").val();
            if(email!="") emailCapture(email);
	   		self.submit();});},onClose:function(dialog){_ltk.Modal.active=false;dialog.container.fadeOut('fast',function(){dialog.overlay.fadeOut('fast',function(){jQuery.ltkmodal.close();jQuery('#ltkmodal-content').remove();});});}});jQuery(document).trigger("ltkmodal.show");}
...
return true;else
return false;}
_ltk.Modal.add=function(opts){this.modals.push(new LTKModal(opts));};_ltk.Modal.load=function(name){if(!_ltk.Modal.suppress()){for(var i=0;i<this.modals.length;i++){if(!this.modals[i].userAgentSuppression()&&((!name&&this.modals[i].autoload)||this.modals[i].name===name)){this.modals[i].load();if(name){return;}}}}};_ltk.Modal.add({ newSessionsOnly: true, initDelay: 3000, followupDelay: 1, autoClose: 0, formHTML: "<!-- Form: BEGIN --> <div id=\"ltkmodal-wrapper\" class=\"signup\"> <div id=\"close-button\"><a href=\"#\" class=\"ltkmodal-close close-button\">X</a></div> <div id=\"ltkmodal-contentarea\" class=\"signup\"> <div id=\"contentInformation\"> <img class=\"sticker\" src=\"http://media.listrak.com/1/Shared/Modals/Images/circle10.png\" alt=\"10% off\" /> <div class=\"float-left\"> <h1>Welcome to Live Superfoods!</h1> <p>Get <strong>10% OFF</strong> right now by signing up as a NEW Live Superfoods email subscriber!</p> <div class=\"form\"> <div class=\"field\"> <input type=\"text\" id=\"ltkmodal-email\" class=\"textbox ltkmodal-email\" name=\"email\" size=\"40\" maxlength=\"100\" value=\"Email Address\" onFocus=\"if (this.value == 'Email Address') this.value = '';\" onBlur=\"if (this.value == '') this.value = 'Email Address';\" /> </div> <div class=\"buttons\"> <a href=\"#\" id=\"signup\" class=\"ltkmodal-subscribe\" title=\"Submit\">Sign Up!</a> <a href=\"#\" class=\"ltkmodal-close ltkmodal-no-thanks\">No Thanks</a></div> <div class=\"field\"> <input type=\"hidden\" class=\"text\" name=\"Source.Modal\" size=\"40\" maxlength=\"100\" value=\"On\" /> </div> </div> <p class=\"small\"><em>*New Subscribers Only.</em></p> </div> <div class=\"clear\"></div> </div> </div> </div> <style type=\"text/css\"> #ltkmodal-container { width:525px; } #ltkmodal-wrapper { position:absolute; width:525px; } #ltkmodal-contentarea { width:100%; font-family:Arial, Helvetica, sans-serif; font-size:13px; color:#3f2e29; text-align:left; overflow:hidden; float:left; background: #ffffff; border: 6px solid #554f3b; border-radius: 10px; box-shadow: 0px 0px 24px rgba(0,0,0,.5); -webkit-box-shadow: 0px 0px 24px rgba(0,0,0,.5); -moz-box-shadow: 0px 0px 24px rgba(0,0,0,.5); } #ltkmodal-contentarea #contentInformation { padding: 20px 20px 15px; margin: 10px; /* border: 5px solid #DDDDDD; border-radius: 5px 5px 5px 5px; */ } #ltkmodal-contentarea .clear { clear:both; } #ltkmodal-wrapper #close-button { position:absolute; top:0; right:0; margin:15px 15px 0 0; } #ltkmodal-wrapper .close-button { font-family:Verdana, Geneva, sans-serif; font-size:11px; font-weight:bold; padding-top:2px; text-decoration:none; text-align:center; color:#ffffff; background-color:#554f3b; border-radius:10px; display:block; width:20px; height:18px; } #ltkmodal-contentarea h1 { font-family: Arial,Helvetica,sans-serif; font-size: 22px; font-weight: bold; color: #336600; margin-top:0; } #ltkmodal-contentarea p { font-family: Arial,Helvetica,sans-serif; font-size: 14px; color: #453135; line-height:18px; margin-top:0; } #ltkmodal-contentarea p.small { font-size: 11px; line-height:16px; } #ltkmodal-contentarea p.reminder { color:#C00; font-weight:bold; } #ltkmodal-contentarea a { color:#453135; } #ltkmodal-contentarea a:hover { text-decoration:none; } #ltkmodal-contentarea .float-left { float:left; width: 315px; padding-right:10px; } #ltkmodal-contentarea .sticker { margin:20px 0; float:right; } #ltkmodal-contentarea .form { clear:both; margin:20px 0 15px; } #ltkmodal-contentarea .field { margin: 10px 0; } #ltkmodal-contentarea input.textbox { color:#666666; display:block; height:30px; border:1px solid #aaaaaa; border-radius:3px; width: 90%; padding-left: 10px; line-height: 30px; vertical-align: middle; margin-top:5px; /* -webkit-box-shadow: 1px 1px 4px 0 #cccccc inset; -moz-box-shadow: 1px 1px 4px 0 #cccccc inset; box-shadow: 1px 1px 4px 0 #cccccc inset; */ } #ltkmodal-contentarea select.dropdown { color:#666666; display:block; height:34px; line-height: 34px; border:1px solid #aaaaaa; border-radius:3px; width: 50%; margin-top:5px; padding: 7px; vertical-align: middle; } #ltkmodal-contentarea .buttons { margin-top: 20px; } #ltkmodal-contentarea .ltkmodal-subscribe { background-color: #336600; border-radius: 3px 3px 3px 3px; color: #FFFFFF; display: inline-block; font-weight: bold; padding: 10px 25px; text-decoration: none; vertical-align: middle; } #ltkmodal-contentarea .ltkmodal-no-thanks { display:inline-block; vertical-align:middle; padding:10px; } @media only screen and (min-width : 320px) and (max-width : 480px), (max-width : 480px) { #ltkmodal-container { left:5% !important; margin:auto !important; width:90% !important; position:absolute !important; } #ltkmodal-wrapper { width:100%; } #ltkmodal-contentarea { text-align:center; } #ltkmodal-contentarea #contentInformation { padding:20px; } #ltkmodal-wrapper #close-button { } #ltkmodal-contentarea .float-left { width:100%; } #ltkmodal-contentarea .form { margin-bottom:0; } #ltkmodal-contentarea .sticker { float:none; margin-top:0; } #ltkmodal-contentarea input.textbox { width:90%; margin:auto; text-align:center; padding-left:0; } #ltkmodal-contentarea select.dropdown { width:75%; } #ltkmodal-contentarea .ltkmodal-subscribe img { width:100px; height:auto; } #ltkmodal-contentarea .ltkmodal-no-thanks img { width:100px; height:auto; } } </style> <!-- Form: END -->", confHTML: "<!-- Confirm: BEGIN --> <div id=\"ltkmodal-wrapper\" class=\"confirm\"> <div id=\"close-button\"><a href=\"#\" class=\"ltkmodal-close close-button\">X</a></div> <div id=\"ltkmodal-contentarea\" class=\"confirm\"> <div id=\"contentInformation\"> <h1>THANKS!</h1> <p>You are now subscribed to receive exclusive discounts, new product updates, and more! Your 10% OFF coupon will be arriving in your inbox momentarily.</p> <p></p> <p class=\"reminder\">REMINDER: If you are already on our email list, you will not be eligible to receive the 10% discount.</p> <p></p> <p>Please <a href=\"#\" class=\"ltkmodal-close\">click here</a> to close this window.</p> </div> <div class=\"clear\"></div> </div> </div> <style type=\"text/css\"> #ltkmodal-contentarea.confirm { } </style> <!-- Confirm: END --> ", redirUrl: "", subCode: "LiveSuperfoods_Modal", urlMode: "0", urlRules: ["https://livesuperfoods.com/checkout/onepage/","https://livesuperfoods.com/checkout/onepage/success/","http://livesuperfoods.com/checkout/cart/"], name: "", testMode: false, autoload: true, overlayClose: true, uaSuppression: "" });
